<script lang="ts">
    ...
    export let conversations: FullConversationType[];

    // funcion de buscar conversaciones
    let term: String;
    ...
    const getConversations= async () =>{
     ...
     // cuando el usuario escribe en el input, hago la peticion a la api.
     async function searchConversations(){
        conversations= await getConversations();
        let group= conversations.filter((conversation) => conversation.name?.toLocaleLowerCase().includes(term.toLocaleLowerCase()));
        let users=  conversations.filter((conversation) => conversation.isGroup === null);
        let usuariosFiltrados: FullConversationType= [];

        users.forEach((conversation) => {
            let usuarios= conversation.users.map((user) => user.name).filter((prueba) => prueba !=  $page.data.session?.user?.name);
            usuarios= usuarios.map((user) => user.toLowerCase());
            if(usuarios.some((user) => user?.toLowerCase().includes(term.toLowerCase()))){
                usuariosFiltrados= usuariosFiltrados.concat(conversation);
            }
        });
        
        conversations= group.concat(usuariosFiltrados);
 ...
 <aside id="logo-sidebar" class="fixed top-0 left-0 z-40 w-[300px] h-screen transition-transform -translate-x-full sm:translate-x-0" aria-label="Sidebar">
    <div class="h-full overflow-y-auto bg-[#120f13]">
       <div class="px-5 flex justify-between items-center h-[65px] shadow-[0px_4px_4px_0px_#00000040]">
            <a class="text-[#E0E0E0] text-[18px] font-bold leading-[25px] -tracking-[0.035em]" href="/conversations">Conversations</a>
            <!-- Modal para crear un canal -->
            <CreateChannel {users}/>
       </div>

       <div class="flex flex-col h-[calc(100%-60px)] justify-between">
           <Search bind:searchValue={term} on:search={searchConversations} on:clear={clearConversations}/>
           <ul class="py-5 px-4 space-y-2 font-medium overflow-auto">
                ...
                {:else}
                    <div class="flex flex-col items-center justify-center h-full gap-[15px]">
                        <h1 class="text-[#E0E0E0] text-lg">Users or conversations not found</h1>
                        <button on:click={() => clearConversations()} class="text-white bg-[#3C46FF] transition-colors duration-200 hover:bg-[#0000FF] rounded-[8px] md:h-[30px] md:w-[80px]">View all</button>
                    </div>
                {/if}    
           </ul>
           ...
           <div class="dropup">
            <button class="flex w-full h-[75px] transitions-colors duration-200 bg-[#0B090C] hover:bg-[#343541] items-center justify-around" id="dropdup-btn">
                ...
                {:else}
                    <img class="h-[42px] w-[42px] rounded-[7px]" src='defaultProfile.jpg' alt="profile image">
                {/if}
                ...
                <div class="dropup-content px-[10.5px] py-[10.5px]">
                    ...
                    <button on:click={signOutToast} class="h-[44px] pl-[11px] w-full text-center items-center inline-flex gap-[15px] text-[#EB5757] -tracking-[0.42px] transitions-colors duration-200 hover:bg-[#3C393F] rounded-[8px]">
